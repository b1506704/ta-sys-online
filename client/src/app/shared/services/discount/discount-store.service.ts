...
import { DiscountHttpService } from './discount-http.service';
import { confirm } from 'devextreme/ui/dialog';

interface DiscountState {
  discountList: Array<Discount>;
  exportData: Array<Discount>;
  selectedDiscount: Object;
...
export class DiscountStore extends StateService<DiscountState> {
  constructor(
    private discountService: DiscountHttpService,
  ...
  fillEmpty(
    startIndex: number,
    endIndex: number,
    sourceArray: Array<Discount>,
    addedArray: Array<Discount>
  ): Array<Discount> {
    let result: Array<Discount> = sourceArray;
  ...
  loadInfiniteDataAsync(page: number, size: number) {
    ...
    this.discountService.fetchDiscount(page, size).subscribe({
      next: (data: any) => {
        this.setState({
          discountList: this.state.discountList.concat(data.data),
        });
  ...
  loadDataAsyncByLearnerID(page: number, size: number, learnerID: string) {
    this.setIsLoading(true);
    this.discountService.fetchDiscountByLearnerID(page, size, learnerID).subscribe({
      next: (data: any) => {
        this.setState({
          discountList: this.fillEmpty(
            page - 1,
            size,
  ...
  initInfiniteDataByLearnerID(page: number, size: number, learnerID: string) {
    return this.discountService
      .fetchDiscountByLearnerID(page, size, learnerID)
      .toPromise()
      .then((data: any) => {
        this.setState({
          discountList: data.data,
  ...
  initData(page: number, size: number) {
    this.discountService
      ...
      .then((data: any) => {
        this.setState({
          discountList: new Array<Discount>(data.totalRecords),
        });
  ...
  initFilterByPropertyData(
    ...
    this.discountService
      .filterDiscountByProperty(property, value, page, size)
      .toPromise()
      .then((data: any) => {
        this.setState({
          discountList: new Array<Discount>(data.totalRecords),
      ...
      .then(() => {
        this.filterDiscountByProperty(property, value, page, size);
      });
  }
  ...
  initSearchByPropertyData(
    ...
    this.discountService
      .searchDiscountByProperty(property, value, page, size)
      .toPromise()
      .then((data: any) => {
        this.setState({
          discountList: new Array<Discount>(data.totalRecords),
      ...
      .then(() => {
        this.searchDiscountByProperty(property, value, page, size);
      });
  }
  ...
  initSortByPropertyData(
    ...
    this.discountService
      .sortDiscountByProperty(value, order, page, size)
      .toPromise()
      .then((data: any) => {
        this.setState({
          discountList: new Array<Discount>(data.totalRecords),
      ...
      .then(() => {
        this.sortDiscountByProperty(value, order, page, size);
      });
  }
  ...
  $discountList: Observable<Array<Discount>> = this.select(
    (state) => state.discountList
  );

  $exportData: Observable<Array<Discount>> = this.select(
  ...
  $selectedDiscount: Observable<Object> = this.select(
    (state) => state.selectedDiscount
  );

  $discountInstance: Observable<Discount> = this.select(
    (state) => state.discountInstance
  );

  uploadDiscount(discount: Discount, page: number, size: number) {
    this.confirmDialog('').then((confirm: boolean) => {
      if (confirm) {
        this.setIsLoading(true);
        this.discountService.uploadDiscount(discount).subscribe({
          next: (data: any) => {
  ...
  updateDiscount(discount: Discount, page: number, size: number) {
    this.confirmDialog('').then((confirm: boolean) => {
      if (confirm) {
        this.setIsLoading(true);
  ...
  deleteSelectedDiscounts(
    selectedDiscounts: Array<string>,
    page: number,
    size: number
  ) {
    this.confirmDialog('').then((confirm: boolean) => {
      if (confirm) {
        this.setIsLoading(true);
        this.discountService.deleteDiscount(selectedDiscounts).subscribe({
          next: (data: any) => {
            this.setState({ responseMsg: data });
            console.log(data);
            this.loadDataAsync(page, size);
  ...
  deleteAll() {
    this.confirmDialog('Delete all items?').then((confirm: boolean) => {
      if (confirm) {
        this.setIsLoading(true);
        this.discountService.deleteAll().subscribe({
          next: (data: any) => {
            this.resetState();
            console.log(data);
            this.setIsLoading(false);
  ...
  deleteDiscount(id: Array<string>, page: number, size: number) {
    this.confirmDialog('').then((confirm: boolean) => {
      if (confirm) {
        this.setIsLoading(true);
  ...
  selectDiscount(_discount: Discount) {
    this.setState({ selectedDiscount: _discount });
  }
  ...
  filterInfiniteDiscountByProperty(
    property: string,
    value: string,
    page: number,
    size: number
    ...
    this.discountService
      ...
      .subscribe({
        next: (data: any) => {
          this.setState({
            discountList: this.state.discountList.concat(data),
          });
          console.log('Filtered list');
          console.log(this.state.discountList);
          console.log('Server response');
  ...
  searchInfiniteDiscountByProperty(
    property: string,
    value: string,
    page: number,
    size: number
  ...
  sortInfiniteDiscountByProperty(
    value: string,
    order: string,
    page: number,
    size: number
  ...
  getDiscount(id: string) {
    ...
    return this.discountService
      ...
      .toPromise()
      .then((data: any) => {
        this.setState({ discountInstance: data });
        console.log(data);
        this.setIsLoading(false);
      });
  }

  setExportData(array: Array<Discount>) {
