...
export class EditSessionComponent implements OnInit, OnDestroy, OnChanges {
  ...
  renderSourceData() {
    ...
    this.getUserMediaData(this.sessionID);
    this.sessionStore.getSession(this.sessionID).then(() => {
  ...
  onSubmit = (e: any) => {
    ...
    this.sessionStore.confirmDialog('').then((confirm: boolean) => {
      if (confirm) {
        ...
        this.sessionHTTP
          .updateSession(this.sessionData)
          .subscribe((data: any) => {
