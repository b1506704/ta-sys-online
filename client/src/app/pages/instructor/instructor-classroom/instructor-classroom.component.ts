...
export class InstructorClassroomComponent implements OnInit, OnDestroy {
  ...
  navigatePracticeOptions: any = {
    ...
    icon: 'refresh',
    hint: 'Fetch data from server',
    onClick: this.navigatePractice.bind(this),
  };
  navigateMemberOptions: any = {
    type: 'normal',
    icon: 'refresh',
    hint: 'Fetch data from server',
    onClick: this.navigateMember.bind(this),
  };
  ...
  navigatePractice() {
    const outlet = {
      outlets: { conditionOutlet: ['practice_list'] },
    };
    this.router.navigate([outlet], { relativeTo: this.route.parent });
  }
  ...
  navigateMember() {
    const outlet = {
      outlets: { conditionOutlet: ['member_list'] },
    };
    this.router.navigate([outlet], { relativeTo: this.route.parent });
  }
  ...
  getMetaData() {
    return this.route.paramMap.subscribe((param) => {
      this.courseData = JSON.parse(param.get('classroom_data'));
      console.log(this.courseData);
      this.courseId = this.courseData.id;
      this.classroomName = this.courseData.name;
      this.store.setCurrentCourseId(this.courseId);
    });
  }
