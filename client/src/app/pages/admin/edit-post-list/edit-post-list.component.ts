...
@Component({
  selector: 'app-edit-post-list',
  templateUrl: './edit-post-list.component.html',
  styleUrls: ['./edit-post-list.component.scss'],
})
export class EditPostListComponent implements OnInit, OnDestroy {
  ...
  onSearchKeyupHandler(e: any) {
    ...
    this.timeout = setTimeout(() => {
      ...
      if (this.currentSearchByPropertyValue !== '') {
        this.postStore.initSearchByPropertyData(
  ...
  onSortValueChanged(e: any) {
    ...
    if (e.value !== '(NONE)') {
      this.postStore.initSortByPropertyData(
  ...
  onFilterChange(e: any) {
    ...
    console.log(e.value);
    if (e.value !== '(NONE)') {
      this.postStore.initFilterByPropertyData(
  ...
  onOptionChanged(e: any) {
    ...
    if (e.fullName === 'paging.pageIndex') {
      ...
      console.log(
        `New page index: ${currentIndex}. Total items: ${this.postList.length}`
      );
      switch (editorMode) {
        case 'NORMAL':
    ...
    if (e.fullName === 'paging.pageSize') {
      ...
      switch (editorMode) {
        case 'NORMAL':
          this.postStore.loadDataAsync(this.currentIndexFromServer, e.value);
          this.goToPage(this.currentIndexFromServer);
          break;
        case 'FILTER':
          this.postStore.filterPostByProperty(
            this.currentFilterProperty,
          ...
          break;
        case 'SEARCH':
          this.postStore.searchPostByProperty(
            this.currentSearchProperty,
            this.currentSearchByPropertyValue,
            this.currentIndexFromServer,
            e.value
          ...
          break;
        case 'SORT':
          this.postStore.sortPostByProperty(
            this.currentSortProperty,
  ...
  paginatePureData(index: number) {
    this.postStore.loadDataAsync(index, this.pageSize);
  }
  ...
  deleteSelectedItems() {
    ...
    const editorMode = this.checkEditorMode();
    if (this.selectedRows.length) {
      this.postStore.confirmDialog('').then((result: boolean) => {
        if (result) {
          this.postHTTP
            .deletePost(this.selectedRows)
            .toPromise()
            .then(() => {
              this.store.showNotif(
      ...
      .then((result: boolean) => {
        if (result) {
          ...
          this.postHTTP
            ...
            .toPromise()
            .then((data: any) => {
              this.postStore.setExportData(data);
              console.log(data);
              setTimeout(() => {
                const workbook = new ExcelJS.Workbook();
                const worksheet = workbook.addWorksheet('Post List');
                exportDataGridToExcel({
