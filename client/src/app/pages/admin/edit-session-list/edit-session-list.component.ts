...
@Component({
  selector: 'app-edit-session-list',
  templateUrl: './edit-session-list.component.html',
  styleUrls: ['./edit-session-list.component.scss'],
})
export class EditSessionListComponent implements OnInit, OnDestroy {
  ...
  onSearchKeyupHandler(e: any) {
    ...
    this.timeout = setTimeout(() => {
      ...
      if (this.currentSearchByPropertyValue !== '') {
        this.sessionStore.initSearchByPropertyData(
  ...
  onSortValueChanged(e: any) {
    ...
    if (e.value !== '(NONE)') {
      this.sessionStore.initSortByPropertyData(
  ...
  onFilterChange(e: any) {
    ...
    console.log(e.value);
    if (e.value !== '(NONE)') {
      this.sessionStore.initFilterByPropertyData(
  ...
  onOptionChanged(e: any) {
    ...
    if (e.fullName === 'paging.pageIndex') {
      ...
      console.log(
        `New page index: ${currentIndex}. Total items: ${this.sessionList.length}`
      );
      switch (editorMode) {
        case 'NORMAL':
    ...
    if (e.fullName === 'paging.pageSize') {
      ...
      switch (editorMode) {
        case 'NORMAL':
          this.sessionStore.loadDataAsync(this.currentIndexFromServer, e.value);
          this.goToPage(this.currentIndexFromServer);
          break;
        case 'FILTER':
          this.sessionStore.filterSessionByProperty(
            this.currentFilterProperty,
          ...
          break;
        case 'SEARCH':
          this.sessionStore.searchSessionByProperty(
            this.currentSearchProperty,
            this.currentSearchByPropertyValue,
            this.currentIndexFromServer,
            e.value
          ...
          break;
        case 'SORT':
          this.sessionStore.sortSessionByProperty(
            this.currentSortProperty,
  ...
  paginatePureData(index: number) {
    this.sessionStore.loadDataAsync(index, this.pageSize);
  }
  ...
  onSaved(e: any) {
    if (e.changes.length) {
      switch (e.changes[0].type) {
        case 'insert':
          this.sessionStore.uploadSession(
            e.changes[0].data,
            this.dataGrid.instance.pageIndex() + 1,
            this.pageSize
          );
  ...
  deleteSelectedItems() {
    ...
    const editorMode = this.checkEditorMode();
    if (this.selectedRows.length) {
      this.sessionStore.confirmDialog('').then((result: boolean) => {
        if (result) {
          this.sessionHTTP
            .deleteSession(this.selectedRows)
            .toPromise()
            .then(() => {
              this.store.showNotif(
      ...
      .then((result: boolean) => {
        if (result) {
          ...
          this.sessionHTTP
            ...
            .toPromise()
            .then((data: any) => {
              this.sessionStore.setExportData(data);
              console.log(data);
              setTimeout(() => {
                const workbook = new ExcelJS.Workbook();
                const worksheet = workbook.addWorksheet('Session List');
                exportDataGridToExcel({
