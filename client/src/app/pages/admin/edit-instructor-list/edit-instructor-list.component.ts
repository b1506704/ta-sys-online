...
@Component({
  selector: 'app-edit-instructor-list',
  templateUrl: './edit-instructor-list.component.html',
  styleUrls: ['./edit-instructor-list.component.scss'],
})
export class EditInstructorListComponent implements OnInit, OnDestroy {
  ...
  onSearchKeyupHandler(e: any) {
    ...
    this.timeout = setTimeout(() => {
      ...
      if (this.currentSearchByNameValue !== '') {
        this.instructorStore.initSearchByNameData(
  ...
  onSortValueChanged(e: any) {
    ...
    if (e.value !== '(NONE)') {
      this.instructorStore.initSortByPriceData(
        e.value,
        this.dataGrid.instance.pageIndex(),
        this.pageSize
      );
  ...
  onFilterChange(e: any) {
    ...
    console.log(e.value);
    if (e.value !== '(NONE)') {
      this.instructorStore.initFilterByCategoryData(
        e.value,
        this.dataGrid.instance.pageIndex(),
        this.pageSize
      );
  ...
  onOptionChanged(e: any) {
    ...
    if (e.fullName === 'paging.pageIndex') {
      ...
      console.log(
        `New page index: ${currentIndex}. Total items: ${this.instructorList.length}`
      );
      switch (editorMode) {
        case 'NORMAL':
    ...
    if (e.fullName === 'paging.pageSize') {
      ...
      switch (editorMode) {
        case 'NORMAL':
          this.instructorStore.loadDataAsync(this.currentIndexFromServer, e.value);
          this.goToPage(this.currentIndexFromServer);
          break;
        case 'FILTER':
          this.instructorStore.filterInstructorByCategory(
          ...
          break;
        case 'SEARCH':
          this.instructorStore.searchInstructorByName(
            this.currentSearchByNameValue,
            this.currentIndexFromServer,
            e.value
          );
          ...
          break;
        case 'SORT':
          this.instructorStore.sortInstructorByPrice(
  ...
  paginatePureData(index: number) {
    if (index === 0) {
      this.instructorStore.loadDataAsync(index, this.pageSize);
      this.instructorStore.loadDataAsync(index + 1, this.pageSize);
    } else {
      this.instructorStore.loadDataAsync(index, this.pageSize);
      this.instructorStore.loadDataAsync(index + 1, this.pageSize);
      this.instructorStore.loadDataAsync(index - 1, this.pageSize);
  ...
  onSaved(e: any) {
    if (e.changes.length) {
      switch (e.changes[0].type) {
        case 'insert':
          this.instructorStore.uploadInstructor(
            e.changes[0].data,
            this.dataGrid.instance.pageIndex(),
            this.pageSize
          );
        ...
        case 'update':
          ...
          this.instructorStore.updateInstructor(
            e.changes[0].data,
            e.changes[0].key,
            this.dataGrid.instance.pageIndex(),
            this.pageSize
          ...
          break;
        case 'remove':
          this.instructorStore.deleteInstructor(
            e.changes[0].key,
            this.dataGrid.instance.pageIndex(),
            this.pageSize
          );
  ...
  deleteSelectedItems() {
    ...
    const editorMode = this.checkEditorMode();
    if (this.selectedRows.length) {
      this.instructorStore.confirmDialog('').then((result: boolean) => {
        if (result) {
          this.instructorHTTP
            .deleteSelectedInstructors(this.selectedRows)
            .toPromise()
            .then(() => {
              this.store.showNotif(
  ...
  onRefresh() {
    ...
    this.isSearchingByName = false;
    this.isSortingByName = false;
    this.instructorStore.initData(this.dataGrid.instance.pageIndex(), this.pageSize);
  }
      ...
      .then((result: boolean) => {
        if (result) {
          ...
          this.instructorHTTP
            ...
            .toPromise()
            .then((data: any) => {
              this.instructorStore.setExportData(data);
              console.log(data);
              setTimeout(() => {
                const workbook = new ExcelJS.Workbook();
                const worksheet = workbook.addWorksheet('instructor List');
                exportDataGridToExcel({
  ...
  deleteAll() {
    this.instructorStore.deleteAllInstructors();
  }
