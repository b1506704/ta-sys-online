...
@Component({
  ...
  styleUrls: ['./edit-schedule-list.component.scss'],
})
export class EditScheduleListComponent implements OnInit, OnDestroy {
  ...
  onSearchKeyupHandler(e: any) {
    ...
    this.timeout = setTimeout(() => {
      ...
      if (this.currentSearchByPropertyValue !== '') {
        this.scheduleStore.initSearchByPropertyData(
  ...
  onSortValueChanged(e: any) {
    ...
    if (e.value !== '(NONE)') {
      this.scheduleStore.initSortByPropertyData(
  ...
  onFilterChange(e: any) {
    ...
    console.log(e.value);
    if (e.value !== '(NONE)') {
      this.scheduleStore.initFilterByPropertyData(
  ...
  onOptionChanged(e: any) {
    ...
    if (e.fullName === 'paging.pageSize') {
      ...
      switch (editorMode) {
        case 'NORMAL':
          this.scheduleStore.loadDataAsync(this.currentIndexFromServer, e.value);
          this.goToPage(this.currentIndexFromServer);
          break;
        case 'FILTER':
          this.scheduleStore.filterScheduleByProperty(
            this.currentFilterProperty,
          ...
          break;
        case 'SEARCH':
          this.scheduleStore.searchScheduleByProperty(
            this.currentSearchProperty,
            this.currentSearchByPropertyValue,
            this.currentIndexFromServer,
            e.value
          ...
          break;
        case 'SORT':
          this.scheduleStore.sortScheduleByProperty(
            this.currentSortProperty,
  ...
  deleteSelectedItems() {
    ...
    if (this.selectedRows.length) {
      this.scheduleStore.confirmDialog('').then((result: boolean) => {
        if (result) {
          this.scheduleHTTP
            .deleteSchedule(this.selectedRows)
            .toPromise()
            .then(() => {
              this.store.showNotif(
