...
import { DiscountHttpService } from 'src/app/shared/services/discount/discount-http.service';
import { DiscountStore } from 'src/app/shared/services/discount/discount-store.service';
import { StoreService } from 'src/app/shared/services/store.service';
import { SubjectStore } from 'src/app/shared/services/subject/subject-store.service';

@Component({
  selector: 'app-edit-discount-list',
  templateUrl: './edit-discount-list.component.html',
  styleUrls: ['./edit-discount-list.component.scss'],
})
export class EditDiscountListComponent implements OnInit, OnDestroy {
  ...
  currentSortProperty: string = 'rate';
  ...
  constructor(
    ...
    private discountHTTP: DiscountHttpService,
  ...
  onSearchKeyupHandler(e: any) {
    ...
    this.timeout = setTimeout(() => {
      ...
      if (this.currentSearchByPropertyValue !== '') {
        this.discountStore.initSearchByPropertyData(
  ...
  onSortValueChanged(e: any) {
    ...
    if (e.value !== '(NONE)') {
      this.discountStore.initSortByPropertyData(
  ...
  onFilterChange(e: any) {
    ...
    console.log(e.value);
    if (e.value !== '(NONE)') {
      this.discountStore.initFilterByPropertyData(
  ...
  onOptionChanged(e: any) {
    ...
    if (e.fullName === 'paging.pageIndex') {
      ...
      console.log(
        `New page index: ${currentIndex}. Total items: ${this.discountList.length}`
      );
      switch (editorMode) {
        case 'NORMAL':
    ...
    if (e.fullName === 'paging.pageSize') {
      ...
      switch (editorMode) {
        case 'NORMAL':
          this.discountStore.loadDataAsync(this.currentIndexFromServer, e.value);
          this.goToPage(this.currentIndexFromServer);
          break;
        case 'FILTER':
          this.discountStore.filterDiscountByProperty(
            this.currentFilterProperty,
          ...
          break;
        case 'SEARCH':
          this.discountStore.searchDiscountByProperty(
            this.currentSearchProperty,
            this.currentSearchByPropertyValue,
            this.currentIndexFromServer,
            e.value
          ...
          break;
        case 'SORT':
          this.discountStore.sortDiscountByProperty(
            this.currentSortProperty,
  ...
  paginatePureData(index: number) {
    this.discountStore.loadDataAsync(index, this.pageSize);
  }
  ...
  onSaved(e: any) {
    if (e.changes.length) {
      switch (e.changes[0].type) {
        case 'insert':
          this.discountStore.uploadDiscount(
            e.changes[0].data,
            this.dataGrid.instance.pageIndex() + 1,
            this.pageSize
          );
        ...
        case 'update':
          ...
          this.discountStore.updateDiscount(
            e.changes[0].data,
            this.dataGrid.instance.pageIndex() + 1,
            this.pageSize
          );
          break;
        case 'remove':
          this.discountStore.deleteDiscount(
            [e.changes[0].key],
            this.dataGrid.instance.pageIndex() + 1,
            this.pageSize
          );
  ...
  deleteSelectedItems() {
    ...
    const editorMode = this.checkEditorMode();
    if (this.selectedRows.length) {
      this.discountStore.confirmDialog('').then((result: boolean) => {
        if (result) {
          this.discountHTTP
            .deleteDiscount(this.selectedRows)
            .toPromise()
            .then(() => {
              this.store.showNotif(
      ...
      .then((result: boolean) => {
        if (result) {
          ...
          this.discountHTTP
            ...
            .toPromise()
            .then((data: any) => {
              this.discountStore.setExportData(data);
              console.log(data);
              setTimeout(() => {
                const workbook = new ExcelJS.Workbook();
                const worksheet = workbook.addWorksheet('Discount List');
                exportDataGridToExcel({
  ...
  sourceDataListener() {
    return this.discountStore.$discountList.subscribe((data: any) => {
      this.discountList = data;
    });
  }

  currentPageListener() {
    return this.discountStore.$currentPage.subscribe((data: any) => {
