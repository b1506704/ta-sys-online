...
import { DiseaseHttpService } from 'src/app/shared/services/disease/disease-http.service';
import { DiseaseStore } from 'src/app/shared/services/disease/disease-store.service';
import { StoreService } from 'src/app/shared/services/store.service';
import diseaseTypeList from 'src/app/shared/services/disease/mock-disease-type';

@Component({
  selector: 'app-edit-disease-list',
  templateUrl: './edit-disease-list.component.html',
  styleUrls: ['./edit-disease-list.component.scss'],
})
export class EditDiseaseListComponent implements OnInit, OnDestroy {
  @ViewChild(DxDataGridComponent, { static: false })
  dataGrid: DxDataGridComponent;
  diseaseList!: Array<Disease>;
  diseaseTypeList: Array<Object> = diseaseTypeList();
  selectedRows: string[];
  ...
  constructor(
    ...
    private diseaseHTTP: DiseaseHttpService,
  ...
  onToolbarPreparing(e: any) {
    e.toolbarOptions.items.unshift(
      ...
      },
      {
        ...
        widget: 'dxSelectBox',
        options: {
          items: this.diseaseTypeList,
          valueExpr: '_id',
          // searchExpr: 'name',
          displayExpr: 'name',
          placeholder: 'Filter with diseaseType',
          // searchEnabled: true,
  ...
  onSearchKeyupHandler(e: any) {
    ...
    this.timeout = setTimeout(() => {
      ...
      if (this.currentSearchByNameValue !== '') {
        this.diseaseStore.initSearchByNameData(
  ...
  onSortValueChanged(e: any) {
    ...
    if (e.value !== '(NONE)') {
      this.diseaseStore.initSortByName(
        e.value,
        this.dataGrid.instance.pageIndex(),
        this.pageSize
      );
  ...
  onFilterChange(e: any) {
    ...
    console.log(e.value);
    if (e.value !== '-1') {
      this.diseaseStore.initFilterByCategoryData(
        e.value,
        this.dataGrid.instance.pageIndex(),
        this.pageSize
      );
  ...
  onOptionChanged(e: any) {
    ...
    if (e.fullName === 'paging.pageIndex') {
      ...
      console.log(
        `New page index: ${currentIndex}. Total items: ${this.diseaseList.length}`
      );
      switch (editorMode) {
        case 'NORMAL':
    ...
    if (e.fullName === 'paging.pageSize') {
      ...
      switch (editorMode) {
        case 'NORMAL':
          this.diseaseStore.loadDataAsync(this.currentIndexFromServer, e.value);
          this.goToPage(this.currentIndexFromServer);
          break;
        case 'FILTER':
          this.diseaseStore.filterDiseaseByCategory(
          ...
          break;
        case 'SEARCH':
          this.diseaseStore.searchDiseaseByName(
            this.currentSearchByNameValue,
            this.currentIndexFromServer,
            e.value
          );
          ...
          break;
        case 'SORT':
          this.diseaseStore.sortDiseaseByName(
  ...
  paginatePureData(index: number) {
    if (index === 0) {
      this.diseaseStore.loadDataAsync(index, this.pageSize);
      this.diseaseStore.loadDataAsync(index + 1, this.pageSize);
    } else {
      this.diseaseStore.loadDataAsync(index, this.pageSize);
      this.diseaseStore.loadDataAsync(index + 1, this.pageSize);
      this.diseaseStore.loadDataAsync(index - 1, this.pageSize);
  ...
  onSaved(e: any) {
    if (e.changes.length) {
      switch (e.changes[0].type) {
        case 'insert':
          this.diseaseStore.uploadDisease(
            e.changes[0].data,
            this.dataGrid.instance.pageIndex(),
            this.pageSize
          );
        ...
        case 'update':
          ...
          this.diseaseStore.updateDisease(
            e.changes[0].data,
            e.changes[0].key,
            this.dataGrid.instance.pageIndex(),
            this.pageSize
          ...
          break;
        case 'remove':
          this.diseaseStore.deleteDisease(
            e.changes[0].key,
            this.dataGrid.instance.pageIndex(),
            this.pageSize
          );
  ...
  deleteSelectedItems() {
    ...
    const editorMode = this.checkEditorMode();
    if (this.selectedRows.length) {
      this.diseaseStore.confirmDialog('').then((result: boolean) => {
        if (result) {
          this.diseaseHTTP
            .deleteSelectedDiseases(this.selectedRows)
            .toPromise()
            .then(() => {
              this.store.showNotif(
      ...
      .then((result: boolean) => {
        if (result) {
          ...
          this.diseaseHTTP
            ...
            .toPromise()
            .then((data: any) => {
              this.diseaseStore.setExportData(data);
              console.log(data);
              setTimeout(() => {
                const workbook = new ExcelJS.Workbook();
                const worksheet = workbook.addWorksheet('Disease List');
                exportDataGridToExcel({
  ...
  sourceDataListener() {
    return this.diseaseStore.$diseaseList.subscribe((data: any) => {
